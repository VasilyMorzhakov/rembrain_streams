...
export class WebSocketImageReceiver implements IImageReceiver {
    ...
    onDataMessage = (ev: MessageEvent<any>) => {
    ...
    async unpackData(data: Blob | string) {
        ...
        depthBlob.arrayBuffer()
            .then(val => Image.load(val), err => console.log("Error while loading depth map: ", err))
            .then(depth => {
                if (!depth) return;
    ...
    onStateMessage = (ev: MessageEvent<any>) => {
